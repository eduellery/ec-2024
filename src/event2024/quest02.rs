...
pub fn part2(notes: &str) -> u32 {
    ...
    let mut symbols: Vec<u32> = vec![0; inscription.len()];

    for word in &words {
        for i in 0..inscription.len() {
            if inscription[i..].starts_with(word) {
                for j in 0..word.len() {
                    symbols[i + j] = 1; // Better than bool, no need to filter and count
                }
            }
        }
    }
...
pub fn part3(notes: &str) -> u32 {
    let (words, inscription) = parse(notes, true);
    let grid = Grid::parse(inscription);
    let mut scales = grid.default_copy();

    for word in &words {
        for x in 0..grid.width {
            for y in 0..grid.height {
                let start = Point::new(x, y);
                find(word, &grid, &mut scales, start, DOWN);
                find(word, &grid, &mut scales, start, RIGHT);
            }
        }
    }

    scales.bytes.iter().sum()
}
